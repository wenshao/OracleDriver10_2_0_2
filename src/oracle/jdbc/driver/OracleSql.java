...
public class OracleSql {
	...
	void initialize(String paramString) throws SQLException {
	...
	void resetNamedParameters() {
		...
		/* 249 */if (this.bindParameterCount != -1) {
			/* 251 */if ((this.cachedBindParameterList == null) || (this.cachedBindParameterList == this.bindParameterList)
	...
	String getSql(boolean paramBoolean1, boolean paramBoolean2) throws SQLException {
		...
		/* 291 */if ((paramBoolean1 != this.currentProcessEscapes) || (paramBoolean2 != this.currentConvertNcharLiterals)) {
			/* 297 */if (paramBoolean2 != this.currentConvertNcharLiterals)
				this.utickSql = null;
	...
	String convertNcharLiterals(String paramString) throws SQLException {
		/* 687 */if (this.lastNcharLiteralLocation <= 2)
			return paramString;
		/* 688 */String str = "";
		/* 689 */int j = 0;
		while (true) {
			...
			/* 696 */if (j >= this.lastNcharLiteralLocation)
				break;
	...
	String generateParameterName() {
		...
		/* 1549 */for (int j = 0;; j++) {
			if (j >= this.parameterList.length)
				break label109;
	...
	public static void main(String[] paramArrayOfString) {
		...
		try {
			...
			/* 1630 */if (localOracleSql.lastNcharLiteralLocation == 2) {
			...
			/* 1642 */if (localOracleSql.whereStartIndex == -1)
			...
			/* 1644 */if (localOracleSql.forUpdateStartIndex == -1)
 ...
 * /jdbc.oracle-10.2.0.2.jar Qualified Name: oracle.jdbc.driver.OracleSql
