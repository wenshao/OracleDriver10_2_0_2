...
/*      */{
	...
	/* 1783 */public static final String[] ModuleName = { "DRVR ", "POOL",
			"DBCV ", "unused", "PIKL ", "JTTC ", "DATM ", "KPRB ", "XA   ",
			"SQLJ ", "JOCI ", "JPUB ", "T2C" };
	...
	/* 1939 */public static final String[] CategoryName = { "OPER ", "PERR ",
			"ERRO ", "WARN ", "FUNC ", "DBG1 ", "DBG2 ", "SQLS " };
	...
	/*      */{
		...
		/*      */{
			...
			/*      */{
				/* 321 */paramPrintWriter
						.println("Oracle Jdbc tracing is not avaliable in a non-debug zip/jar file");
	...
	/*      */{
		...
		/*      */{
			...
			/*      */
			/* 388 */conversionLogger = Logger
					.getLogger("oracle.jdbc.conversion");
			/*      */
	...
	/*      */{
		...
		/*      */{
			/* 447 */localStreamHandler = printHandler;
			/* 448 */AccessController.doPrivileged(new PrivilegedAction(
					localStreamHandler) {
				/*      */private final StreamHandler val$fprintHandler;

				/*      */
				/* 452 */public Object run() {
					OracleLog.rootLogger.removeHandler(this.val$fprintHandler);
					/* 453 */return null;
				}
				/*      */
			});
			/*      */}
		/* 457 */printHandler = new StreamHandler(paramPrintStream,
				new SimpleFormatter());
		/*      */
		...
		/* 461 */AccessController.doPrivileged(new PrivilegedAction(
			...
			/* 465 */public Object run() {
				...
				/* 466 */OracleLog.rootLogger
						.addHandler(this.val$fprintHandler2);
				/* 467 */return null;
				/*      */}
			/*      */
		});
	...
	/*      */{
		...
		/*      */case 3:
			...
			/* 552 */break;
		/*      */default:
			/* 555 */print(null, 1, 2, 4, "Logging volume level " + paramInt
					+ " is not in the range of 1 to 3");
			/*      */
			/* 558 */return;
			/*      */}
		/*      */
	...
	/*      */{
		...
		/* 660 */AccessController
				.doPrivileged(new PrivilegedAction(localLevel2) {
		...
		/* 669 */int i = paramInt1;
		/* 670 */AccessController.doPrivileged(new PrivilegedAction(i,
			...
			/* 674 */public Object run() {
				OracleLog.driverLogger
						.setLevel((this.val$fmoduleMask & 0x1) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 676 */OracleLog.poolLogger
						.setLevel((this.val$fmoduleMask & 0x2) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 678 */OracleLog.conversionLogger
						.setLevel((this.val$fmoduleMask & 0x4) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 680 */OracleLog.adtLogger
						.setLevel((this.val$fmoduleMask & 0x10) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 682 */OracleLog.thinLogger
						.setLevel((this.val$fmoduleMask & 0x20) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 684 */OracleLog.datumLogger
						.setLevel((this.val$fmoduleMask & 0x40) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 686 */OracleLog.kprbLogger
						.setLevel((this.val$fmoduleMask & 0x80) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 688 */OracleLog.xaLogger
						.setLevel((this.val$fmoduleMask & 0x100) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 690 */OracleLog.sqljLogger
						.setLevel((this.val$fmoduleMask & 0x200) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 692 */OracleLog.ociLogger
						.setLevel((this.val$fmoduleMask & 0x400) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 694 */OracleLog.jpubLogger
						.setLevel((this.val$fmoduleMask & 0x800) != 0 ? this.val$flevel
								: this.val$off);
				/*      */
				/* 696 */return null;
				/*      */}
	...
	/*      */{
		...
		/*      */
		/* 726 */print(null, 1, 2, 1, "Set logging sub-mask for module "
				+ getMaskHexStr(moduleMask) + "(number " + i + ") to "
				+ getMaskHexStr(paramInt2));
		/*      */}
	...
	/*      */{
		/* 748 */if (paramInt > 0) {
			...
			/*      */}
		/* 751 */print(null, 1, 2, 1,
				"Set the maximum number of bytes to be printed to "
						+ maxPrintBytes);
		/*      */}

	/*      */
	/*      */public static boolean registerClassNameAndGetCurrentTraceSetting(
			Class paramClass)
	/*      */{
		/* 763 */return false;
		/*      */}
	...
	/*      */
	/*      */public static boolean registerClassNameAndGetCurrentPrivateTraceSetting(
			Class paramClass)
	/*      */{
		/* 770 */return false;
		/*      */}
	...
	/*      */{
		...
		/*      */{
			/* 818 */System.out.println("ERROR: OracleLog.ModuleName[] has "
					+ ModuleName.length + " items (expected " + 13 + ")");
			/*      */}
		/*      */
		/* 823 */if (CategoryName.length != 8)
		/*      */{
			/* 825 */System.out.println("ERROR: OracleLog.CategoryName[] has "
					+ ModuleName.length + " items (expected " + 13 + ")");
			/*      */}
		/*      */}
		...
		/*      */{
			...
			/*      */{
				...
				/*      */{
					...
					/*      */
					/* 861 */localPrintStream = new PrintStream(
			...
			/*      */{
				/* 921 */str = getSystemProperty("oracle.jdbc.SubmodMask" + j,
						null);
				/*      */
				/* 924 */if (str == null)
					/*      */continue;
	...
	/*      */
	/*      *//** @deprecated */
	/*      */private static String getSystemProperty(String paramString1,
			String paramString2)
	/*      */{
		...
		/*      */{
			...
			/* 986 */AccessController.doPrivileged(new PrivilegedAction(
				...
				/* 990 */public Object run() {
					this.val$retStr[0] = System.getProperty(this.val$fstr,
	...
	/*      */
	/*      *//** @deprecated */
	/*      */public static void print(Object paramObject, int paramInt1,
	...
	/*      */{
		...
		/* 1061 */int i = 0;
		/*      */
		/* 1072 */if (((paramInt1 & moduleMask) != 0)
				&& ((paramInt2 & submodMasks[getBitNumFromVector(paramInt1)]) != 0)
				&& ((paramInt3 & categoryMask) != 0))
		/*      */{
			/* 1076 */i = 1;
			/*      */}
		/*      */
		...
		/*      */{
			...
			/* 1102 */if ((printMask & 0x8) != 0) {
				/* 1103 */localStringBuffer
						.append(getBitNumFromVector(paramInt2) + " ");
				/*      */}
			/* 1105 */if ((printMask & 0x10) != 0) {
				/* 1106 */localStringBuffer.append(getCategoryName(paramInt3));
				/*      */}
			/* 1108 */if ((printMask & 0x40) != 0) {
				/* 1109 */localStringBuffer.append(Thread.currentThread() + "_"
	...
	/*      */{
		...
		/* 1195 */PrintWriter localPrintWriter = new PrintWriter(
				localStringWriter);
		/*      */
		/* 1197 */paramException.printStackTrace(localPrintWriter);
		/* 1198 */print(paramObject, paramInt1, paramInt2, paramInt3,
	...
	/*      */{
		...
		/* 1231 */if (isEnabled())
		/*      */{
			/* 1233 */str = "Enabled logging (moduleMask "
					+ getMaskHexStr(moduleMask) + ", categoryMask "
					+ getMaskHexStr(categoryMask) + ")";
			/*      */}
		/*      */else
		/*      */{
	...
	/*      */
	/*      *//** @deprecated */
	/*      */public static String bytesToPrintableForm(String paramString,
			byte[] paramArrayOfByte)
	/*      */{
		/* 1491 */int i = paramArrayOfByte == null ? 0
	...
	/*      */{
		...
		/*      */else {
			/* 1526 */str = paramString + " (" + paramArrayOfByte.length
					+ " bytes):\n"
	...
	/*      */
	/*      *//** @deprecated */
	/*      */public static String bytesToFormattedStr(byte[] paramArrayOfByte,
	...
	/*      */{
		/* 1643 */if (paramArrayOfChar == null) {
			/* 1644 */return null;
			/*      */}
		/* 1646 */return charsToUcs2Bytes(paramArrayOfChar,
	...
	/*      */{
		...
		/*      */case 5:
			...
			/* 2142 */break;
		/*      */case 6:
			/* 2145 */str = "000000000000"
					+ Long.toString(paramLong & 0xFFFFFFFF, 16);
			/*      */
			/* 2147 */break;
		/*      */case 7:
			/* 2150 */str = "00000000000000"
					+ Long.toString(paramLong & 0xFFFFFFFF, 16);
			/*      */
			/* 2153 */break;
		/*      */case 8:
			/* 2156 */return toHex(paramLong >> 32, 4)
